...
def apply_rotary_emb(x, cos, sin):
    ...
    y1 = x1 * cos.to(x.device) + x2 * sin.to(x.device)  # rotate pairs of dims
    y2 = x1 * (-sin.to(x.device)) + x2 * cos.to(x.device)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx, causal: bool = True):
        super().__init__()
    ...
    def forward(self, x, cos_sin, kv_cache):
        ...
        if kv_cache is None or Tq == Tk:
            ...
            y = F.scaled_dot_product_attention(
                q, k, v, is_causal=self._causal, enable_gqa=enable_gqa
            )
        elif Tq == 1:

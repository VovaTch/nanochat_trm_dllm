...
model_config_kwargs = dict(
    ...
    sup_steps=supervision_steps,
    y_loop=y_loop_steps,
    z_loop=z_loop_steps,
)

with torch.device("meta"):
    model_config = TRDLMConfig(**model_config_kwargs)
    model = TRDLM(model_config)
model.to_empty(device=device)
model.init_weights()
orig_model = model  # original, uncompiled model, for saving raw model state_dict
# model = torch.compile(model, dynamic=False)  # TODO: dynamic True/False think through
num_params = sum(p.numel() for p in model.parameters())  # type: ignore
print0(f"Number of parameters: {num_params:,}")
num_flops_per_token = model.estimate_flops()  # type: ignore
...
optimizers = model.setup_optimizers(  # type: ignore
...
train_loader = tokenizing_distributed_data_loader(
    device_batch_size, max_seq_len, split="train", device=device.type
)


def build_val_loader() -> Generator[tuple[torch.Tensor, torch.Tensor], None, None]:
    return tokenizing_distributed_data_loader(
        device_batch_size, max_seq_len, split="val", device=device.type
    )
...
for step in range(num_iterations + 1):
    ...
    for micro_step in range(grad_accum_steps):
        accum_target.append(x.clone())
        accum_y_inner.append(model.y_init.repeat((x.shape[0], x.shape[1], 1)).to(x.device))  # type: ignore
